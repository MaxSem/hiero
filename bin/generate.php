#!/usr/bin/php
<?php

declare(strict_types=1);

if (PHP_SAPI !== 'cli') {
    die;
}

const UNICODE_VERSION = '17.0.0';
const FIRST_CHAR = 0x13000;
const LAST_CHAR = 0x1342F;

$tmpDir = sys_get_temp_dir();
$destDir = dirname(__DIR__) . '/data';

$charToGardiner = readUnicodeData(downloadUnicodeData($tmpDir));
$gardinerToChar = array_flip($charToGardiner);
$categories = getHieroglyphCategories($charToGardiner);

saveData(
    "$destDir/unicode.php",
    [
        'charToGardiner' => $charToGardiner,
        'gardinerToChar' => $gardinerToChar,
        'categories' => $categories,
    ]
);

echo "Generating phonetics arrays.\n";
$phoneticToGardiner = readPhonetics("$destDir/phonetics.txt", $gardinerToChar);

$phoneticsLc = makePhoneticsLc($phoneticToGardiner);

saveData(
    "$destDir/phonetics.php",
    [
        'phoneticToGardiner' => $phoneticToGardiner,
        'lowerCaseIndex' => $phoneticsLc,
    ]
);

function bail(string $message): void
{
    fputs(STDERR, "$message\n");
    exit(1);
}

function downloadUnicodeData(string $tmpDir): string
{
    $remoteUrl = sprintf('https://www.unicode.org/Public/%s/ucd/UnicodeData.txt', UNICODE_VERSION);
    $localPath = sprintf('%s/UnicodeData-%s.txt', $tmpDir, UNICODE_VERSION);

    if (is_file($localPath) && is_readable($localPath) && filesize($localPath) > 2 * 1024 * 1024) {
        echo "Unicode data is already present at $localPath\n";
        return $localPath;
    }

    echo "Downloading Unicode data file...\n";

    ini_set('user_agent', 'Mozilla/4.0 (compatible; MSIE 5.01; Windows 95)');
    $contents = file_get_contents($remoteUrl);

    if ($contents === false) {
        $err = error_get_last()['message'];
        bail("Error downloading Unicode data file: $err");
    }

    if (strlen($contents) < 1024 * 1024) {
        echo $contents;
        exit(1);
    }

    file_put_contents($localPath, $contents);

    echo "Download complete.\n";

    return $localPath;
}

function readUnicodeData(string $filename): array
{
    $result = [];
    foreach (file($filename) as $line) {
        $line = preg_replace('/#.*$/', '', $line);
        $line = trim($line);

        $parts = explode(';', $line);
        if (count($parts) < 8) {
            continue;
        }

        $charNo = (int)base_convert($parts[0], 16, 10);
        
        if ($charNo < FIRST_CHAR) {
            continue;
        }
        if ($charNo > LAST_CHAR) {
            break;
        }

        if (!preg_match('/EGYPTIAN HIEROGLYPH (A?[A-Z][0-9][0-9][0-9][A-Z]?)/', $parts[1], $matches)) {
            continue;
        }

        $name = ucfirst(strtolower($matches[1]));
        $name = preg_replace('/(?<=[a-z])0+/i', '', $name);

        $result[$charNo] = $name;
    }

    printf("%d characters found.\n", count($result));
    if (count($result) < 100) {
        bail('Problem with Unicode definitions/parsing?');
    }

    return $result;
}

function getHieroglyphCategories(array $unicodeData): array
{
    $categories = [];
    foreach ($unicodeData as $code) {
        $group = preg_replace('/^(Aa|[A-Z]).*$/', '$1', $code);
        $categories[$group][] = $code;
    }

    return $categories;
}

/**
 * @param string $filename
 * @param array<string, string> $gardinerToChar
 * @return array<string, string>
 */
function readPhonetics(string $filename, array $gardinerToChar): array
{
    $lines = file($filename) or bail("Could not read $filename");

    $result = [];
    foreach ($lines as $line) {
        $line = trim($line);

        if ($line === '' || $line[0] === '#') {
            continue;
        }

        $parts = preg_split('/\s*=\s*/', $line, 2, PREG_SPLIT_NO_EMPTY);
        if (count($parts) !== 2) {
            bail("Could not parse line '$line'");
        }

        $phoneme = $parts[1];
        $gardiner = ucfirst($parts[0]);

        if (!isset($gardinerToChar[$gardiner])) {
            // Phonemes have some codes like J* that are present neither in actual Gardiner's list nor in Unicode
            continue;
        }

        $result[$phoneme] = $gardiner;
    }

    return $result;
}

/**
* @param array<string|int, string> $phoneticToGardiner
* @return array<string, string>
 */
function makePhoneticsLc(array $phoneticToGardiner): array
{
    $result = [];

    // First, fill the array with lowercased phonemes
    foreach ($phoneticToGardiner as $phoneme => $gardiner) {
        $lower = strtolower((string)$phoneme);
        if ($lower !== $phoneme) {
            $result[$lower] = $phoneme;
        }
    }

    // Then, overwrite conflicting lowercased phonemes with those naturally lowercase
    // so that the latter had higher precedence.
    foreach ($phoneticToGardiner as $phoneme => $gardiner) {
        $lower = strtolower((string)$phoneme);
        if ($lower === $phoneme) {
            $result[$lower] = $phoneme;
        }
    }

    return $result;
}

function saveData(string $filename, array $data): void
{
    $timestamp = (new DateTime())->format('Y-m-d H:i:s');
    $content = <<<PHP
    <?php
    // DO NOT EDIT, this is an automatically generated file.
    // Generated by bin/generate.php on $timestamp


    PHP;

    $content .= 'return ' . var_export($data, true) . ";\n";

    file_put_contents($filename, $content);
}


